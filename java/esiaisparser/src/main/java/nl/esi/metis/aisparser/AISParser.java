...
/** This class supports parsing of AIS messages.
 * It can be used as follows:
 * <ul>
 * <li> Define a class that implements interface {@link HandleAISMessage}. This should do something useful with the AIS messages that it gets.</li>
 * <li> Define a class that implements interface {@link HandleInvalidInput}. This could ignore, or do something useful (like logging or estimating sensor health) with the invalid input.</li>
 * <li> Create an instance of {@link AISParser}, using an instance of the above AIS message handler class.</li>
 * <li> Feed the parser with sensor data, either using the {@link ReadPath#readFile readFile} method (that assumes lines of sensor data according to {@link ReadPath#linePattern}) 
 * or piece of sensor data at a time using the {@link #handleSensorData handleSensorData} method. 
 *      Now each time an AIS message is complete, it will be fed to the AIS message handler.</li>
 * </ul>
...
public class AISParser {
	/** The line handler that will be invoked for each line that is fed to the parser. */
	private HandleSensorData lineHandler = null;
	
	/** Constructs an AIS parser object.
	 * This object will use the provided AIS message handler to deal with every AIS message it will find.
	 ...
	 * The line must be passed together with the provenance information.
	 * This is useful for situations where live AIS information arrives spread out over time.
	...
	/** Signals that the handling of individual lines is finished.
	 * This method should be called after a number of lines are passed to the {@link #handleSensorData(Provenance, String) handleLine} method.
	 */
	 ...
	 * This method just counts the annotated AIS messages in a single file or in a whole directory of AIS files.
	 * For this purpose it uses a simple private message handler class.
	...
	static private class MyAISMessageHandler implements HandleAISMessage {
		...
		/** Handles a single AIS message. 
		 * In this case, if the message has type 1 the count is incremented.
		...
		public void handleAISMessage(AISMessage message) {
			...
			if (annotations.size() > 0)
			{
				...
				for(Annotation annotation : annotations)
				{
					...
					if (annotation instanceof AISIllegalValueAnnotation)
					{
						illegalValueAnnotationCount++;
					} else
					if (annotation instanceof ChannelIdAnnotation)
					{
						channelIDAnnotationCount++;
					} else
					if (annotation instanceof ChangedChannelIdAnnotation)
					{
						changedChannelIdAnnotationCount ++;
					} else
					if (annotation instanceof AISInconsistentLengthForTypeAnnotation)
					{
						inconsistentLengthForTypeAnnotationCount ++;
					} else
					if (annotation instanceof AISHypothesisAnnotation)
					{
